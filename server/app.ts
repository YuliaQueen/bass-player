 ...
 */
const decodeFilename = (originalname: string): string => Buffer.from(originalname, 'latin1').toString('utf8');

export interface CreateAppOptions {
    /** Абсолютный путь к папке с .gp файлами. */
    tabsDir: string;
}

interface TabFileInfo {
    name: string;
    size: number;
    mtime: Date;
}
 ...
 */
export const createApp = ({ tabsDir }: CreateAppOptions): Express => {
    const app = express();
    app.use(cors());
    ...
    const listTabs = async (): Promise<TabFileInfo[]> => {
        const files = await fs.readdir(tabsDir);
        const tabs = await Promise.all(
            files
                .filter((name) => ALLOWED_EXT.test(name))
                .map(async (name): Promise<TabFileInfo> => {
    ...
    app.get('/api/tabs', async (_req: Request, res: Response) => {
        try {
    ...
    app.post('/api/tabs', (req: Request, res: Response, _next: NextFunction) => {
    ...
    app.delete('/api/tabs/:name', async (req: Request, res: Response) => {
    ...
    app.get('/tabs/:name', (req: Request, res: Response) => {
        const safeName = path.basename(req.params.name ?? '');
